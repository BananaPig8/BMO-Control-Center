(() => {
  ...
  function renderPantallasBody() {
    ...
    document.getElementById("kiosk-restart-btn")?.addEventListener("click", async (e) => {
      if (!askConfirm("¿Reiniciar el navegador del kiosco? La pantalla se recargará.")) return;
      e.target.disabled = true;
  ...
  function renderUiPagesList() {
    ...
    list.querySelectorAll("[data-ui-page-delete]").forEach((btn) => {
      btn.addEventListener("click", async () => {
        ...
        const page = (state.uiPages || []).find((p) => p.id === id);
        if (!askConfirm(`¿Eliminar la página "${page?.name || ""}"?`)) return;
        try {
  ...
  function renderProfilesList() {
    ...
    container.querySelectorAll("[data-profile-delete]").forEach((btn) => {
      btn.addEventListener("click", async () => {
        ...
        if (!askConfirm(`¿Eliminar el perfil "${profile?.name || ""}"?`)) return;
        try {
  ...
  // 4.11.17 - Confirmacion SIEMPRE real (window.confirm), igual que en la
  // importacion de backups de 4.10.14: esto borra dock personalizado,
  // paginas, perfiles y fondo de golpe - no se salta con "confirmar
  // acciones destructivas: no" (4.10.12), es la accion mas destructiva de
  // toda la personalizacion.
  document.getElementById("ui-restore-factory-btn")?.addEventListener("click", async (e) => {
    if (!window.confirm("¿Restaurar el diseño de fábrica de BMO? Se perderán el dock personalizado, las páginas, los perfiles y el fondo que hayas configurado. Esto no se puede deshacer.")) {
      return;
    }
    e.target.disabled = true;
  ...
  function anyOverlayOpen() {
    return !!document.querySelector(".sys-sub-panel:not([hidden])") || !!(els.frame && !els.frame.hidden);
  }
  ...
  function initGlobalGestures() {
    ...
    function inGuardedZone(target) {
      return !!target.closest(".services-pager, .services-grid.reordering, .sys-sub-panel, #app-nav, #app-frame");
    }
